...
import { TeachingContract, TeachingContractCourse, CourseMapping, Course, LecturerProfile } from '../model/index.js';
...
export async function getMyCourseMappings(req, res) {
  ...
  const contractCourses = await TeachingContractCourse.findAll({
    ...
    const normalizeTerm = (t) => String(t ?? '')
      .toLowerCase()
      .replace(/^term\s*/, '')
      .trim();
    ...
    const keyOf = (c) => `${normCourseId(c.course_id)}|${normalizeTerm(c.term)}|${String(c.academic_year ?? '').trim()}`;
  ...
  const contract_total_hours = contract_id != null ? (totalHoursByContractId.get(contract_id) || 0) : null;
      const parts = [];
      ...
      const thNum = toNumber(r.theory_hours) ?? (r.type_hours?.includes('15h') ? 15 : r.type_hours?.includes('30h') ? 30 : null);
