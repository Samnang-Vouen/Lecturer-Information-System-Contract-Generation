...
import { TeachingContract, TeachingContractCourse, LecturerProfile, User, Candidate } from '../model/index.js';
...
export async function getLecturerSalaryAnalysis(req, res) {
  try {
    ...
    const lecturerUser = await User.findByPk(lecturerUserId, { attributes: ['id','email','display_name'] });
    ...
    try {
      ...
      if (!cand && lecturerUser?.display_name) {
        ...
        const cleaned = lecturerUser.display_name.replace(titleRegex, '').replace(/\s+/g, ' ').trim().toLowerCase();
        cand = await Candidate.findOne({ where: Sequelize.where(Sequelize.fn('LOWER', Sequelize.fn('TRIM', Sequelize.col('fullName'))), cleaned) });
      }
  ...
  const contracts = await TeachingContract.findAll({
    ...
    let totalHours = 0; let totalUsd = 0; let totalKhr = 0;

    const monthKey = (d) => new Date(d).toISOString().slice(0,7);
    const enumerateMonths = (start, end) => {
      const out = [];
      if (!start || !end) return out;
      const s = new Date(start); s.setDate(1); s.setHours(0,0,0,0);
      const e = new Date(end); e.setDate(1); e.setHours(0,0,0,0);
      while (s <= e) {
    ...
    for (const c of contracts) {
      const hours = (c.courses || []).reduce((a, cur) => a + (Number.isFinite(+cur.hours) ? +cur.hours : 0), 0);
      ...
      if (hasRange) { totalHours += hours; totalUsd += amountUsd; totalKhr += amountKhr; }
      const label = `${c.academic_year || ''}${c.term ? ` • Term ${c.term}` : ''}`.trim() || `#${c.id}`;
      byContract.push({ contractId: c.id, label, hours, amountUsd, amountKhr, start_date: c.start_date, end_date: c.end_date });
    ...
    const byMonthArr = Array.from(byMonth.entries()).sort((a,b)=>a[0].localeCompare(b[0])).map(([month, v]) => ({ month, usd: Math.round(v.usd), khr: Math.round(v.khr) }));

    return res.json({
      currency: 'KHR',
      exchangeRate: rate,
      hourlyRateUsd,
      totals: { contracts: contracts.length, hours: totalHours, usd: Math.round(totalUsd), khr: Math.round(totalKhr) },
      byContract,
      byMonth: byMonthArr
    });
  } catch (e) {
