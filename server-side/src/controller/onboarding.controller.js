...
export const submitOnboarding = async (req, res) => {
  try {
    ...
    const folderSlug = (existing?.storage_folder || (body.full_name_english || user.display_name || user.email.split('@')[0])
    ...
    const cv_path = await saveFile(cvFile, 'cv'+ (cvFile?.originalname? path.extname(cvFile.originalname):'.pdf'));
    const course_syllabus = await saveFile(syllabusFile, 'syllabus'+ (syllabusFile?.originalname? path.extname(syllabusFile.originalname):'.pdf'));
    const pay_roll_in_riel = await saveFile(payrollFile, 'payroll'+ (payrollFile?.originalname? path.extname(payrollFile.originalname):'.pdf'));
  ...
  research_fields: Array.isArray(body.research_fields) ? body.research_fields.join(',') : body.research_fields || null,
  ...
  let unmatchedCourses = [];
    try {
      ...
      if(body.departments){
        const deptNamesRaw = Array.isArray(body.departments) ? body.departments : body.departments.split(',');
      ...
      let courseIds = [];
      if(body.course_ids){
        const raw = Array.isArray(body.course_ids) ? body.course_ids : String(body.course_ids).split(',');
      ...
      if(courseIds.length){
        ...
        const filteredCourses = deptIdSet.size ? allCourses.filter(c=> deptIdSet.has(c.dept_id)) : allCourses;
        if(filteredCourses.length){
          await LecturerCourse.destroy({ where: { lecturer_profile_id: profile.id } });
          await LecturerCourse.bulkCreate(filteredCourses.map(c=>({ lecturer_profile_id: profile.id, course_id: c.id })));
        ...
        if(courseNames.length){
          ...
          courseNames.forEach(inp=>{ const key=normalize(inp); const m = cMap.get(key); if(m && !matchedCourses.find(x=>x.id===m.id)) matchedCourses.push(m); else if(!m) unmatchedCourses.push(inp); });
          if(matchedCourses.length){
            await LecturerCourse.destroy({ where: { lecturer_profile_id: profile.id } });
            await LecturerCourse.bulkCreate(matchedCourses.map(c=>({ lecturer_profile_id: profile.id, course_id: c.id })));
