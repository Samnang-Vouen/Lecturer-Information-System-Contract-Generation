...
async function ensureContractItemsSchema() {
  ...
  try {
    await sequelize.query("ALTER TABLE `contract_items` ADD CONSTRAINT `fk_contract_items_contract` FOREIGN KEY (`contract_id`) REFERENCES `Teaching_Contracts`(`id`) ON DELETE CASCADE");
  } catch {}
}
