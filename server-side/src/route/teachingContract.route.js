...
import { createDraftContract, getContract, generatePdf, updateStatus, upload, uploadSignature, listContracts, deleteContract } from '../controller/teachingContract.controller.js';
...
router.get('/:id/pdf', authorizeRoles(['admin','lecturer','management','superadmin']), generatePdf);

// Update status (lecturer or management)
router.patch('/:id/status', authorizeRoles(['admin','lecturer','management','superadmin']), updateStatus);

// Upload e-signature (who=lecturer|management)
router.post('/:id/signature', authorizeRoles(['admin','lecturer','management','superadmin']), upload.single('file'), uploadSignature);
