...
try {
    ...
    await InterviewQuestion.sync({ alter: false });
        ...
        await sequelize.query("UPDATE `Interview-Questions` SET `canonical_text` = LOWER(TRIM(REGEXP_REPLACE(question_text, ' +', ' '))) WHERE `canonical_text` = '' OR `canonical_text` IS NULL");
