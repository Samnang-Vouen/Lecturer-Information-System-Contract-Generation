...
async function run(){
  try {
    ...
    const telecomOld = await Department.findOne({ where: { dept_name: 'Telecommunications and Network' } });
    let telecom = await Department.findOne({ where: { dept_name: 'Telecommunications and Networking' } });
    ...
    const ensureCourse = async ({ code, name, dept }) => {
      ...
      if(!c){
        c = await Course.create({ dept_id: dept.id, course_code: code, course_name: name, credits: 3, hours: 45 });
    ...
    const advNet = await ensureCourse({ code: 'NET401', name: 'Advanced Networking', dept: telecom });
    const oop = await ensureCourse({ code: 'CS302', name: 'Object-Oriented Programming', dept: cs });
    ...
    if(toAdd.length){
      await LecturerCourse.bulkCreate(toAdd.map(course_id=>({ lecturer_profile_id: profileId, course_id })));
